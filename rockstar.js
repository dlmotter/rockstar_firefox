(function () {
            ...
            function getSSO(url) {
                ...
                getJSON(url).then(response => {
                    ...
                    if (matches = response.match(/name="(SAMLResponse|wresult)".*value="(.*?)"/)) {
                        ...
                        ssoPopup.html("Post to: " + postTo + "<br><br><pre style='font-family: Monaco, Consolas, \"Lucida Console\", monospace; white-space: pre-wrap; word-break: break-all;'>" + indentXml(assertion, 4) + "</pre>");
                        ...
                        ssoPopup.html(`<pre style='font-family: Monaco, Consolas, "Lucida Console", monospace; white-space: pre-wrap; word-break: break-all;'>${form}</pre>`);
                    } else if (matches = response.match(/<div class="error-content">(?:.|\n)*?<\/div>/)) {
                        ssoPopup.html(`<pre style='font-family: Monaco, Consolas, "Lucida Console", monospace; white-space: pre-wrap; word-break: break-all;'>${matches[0]}</pre>`);
                    } else {
    ...
    function apiExplorer() {
        createDiv("API Explorer", mainPopup, function () {
            ...
            datalist.id = "urls";
            const paths = 'agentPools,agentPools/${poolId},apps,apps/${appId},apps/${appId}/credentials/keys,apps/${appId}/credentials/keys/${keyId},apps/${appId}/features,apps/${appId}/grants,apps/${appId}/groups,apps/${appId}/groups/${groupId},apps/${appId}/lifecycle/activate,apps/${appId}/lifecycle/deactivate,apps/${appId}/policies,apps/${appId}/tokens,apps/${appId}/users,apps/${appId}/users/${userId},apps?filter=user.id eq "${userId}",apps?expand=user/${userId}&limit=200,' + 
                'authenticators,authenticators/${authenticatorId},authenticators/${authenticatorId}/lifecycle/activate,authenticators/${authenticatorId}/lifecycle/deactivate,authorizationServers,authorizationServers/${authServerId},authorizationServers/${authServerId}/claims,authorizationServers/${authServerId}/clients,authorizationServers/${authServerId}/policies,authorizationServers/${authServerId}/scopes,' + 
                'behaviors,behaviors/${behaviorId},brands,brands/${brandId},brands/${brandId}/themes,captchas,captchas/${captchaId},devices,devices/${deviceId},devices/${deviceId}/lifecycle/activate,devices/${deviceId}/lifecycle/deactivate,devices/${deviceId}/lifecycle/suspend,devices/${deviceId}/lifecycle/unsuspend,devices/${deviceId}/users,' + 
                'domains,domains/${domainId},domains/${domainId}/certificate,eventHooks,eventHooks/${eventHookId},eventHooks/${eventHookId}/lifecycle/activate,eventHooks/${eventHookId}/lifecycle/deactivate,eventTypes,features,groups,groups/${groupId},groups/${groupId}/apps,groups/${groupId}/lifecycle/activate,groups/${groupId}/lifecycle/deactivate,groups/${groupId}/roles,groups/${groupId}/users,groups/rules,groups/rules/${ruleId},' + 
                'idps,idps/${idpId},idps/${idpId}/credentials/keys,idps/${idpId}/lifecycle/activate,idps/${idpId}/lifecycle/deactivate,idps/${idpId}/users,inlineHooks,inlineHooks/${inlineHookId},inlineHooks/${inlineHookId}/lifecycle/activate,inlineHooks/${inlineHookId}/lifecycle/deactivate,inlineHooks/types,' + 
                'logs,mappings,mappings/${mappingId},meta/schemas/apps/${instanceId}/default,meta/schemas/group/default,meta/schemas/user/default,meta/schemas/user/linkedObjects,meta/types/user,myaccount/emails,myaccount/phones,myaccount/profile,' + 
                'networkZones,networkZones/${zoneId},org,org/contacts,org/factors,org/factors/yubikey_token/tokens,org/logo,org/preferences,org/privacy,policies,policies/${policyId},policies/${policyId}/lifecycle/activate,policies/${policyId}/lifecycle/deactivate,policies/${policyId}/rules,policies/${policyId}/rules/${ruleId},policies?type=${type},' + 
                'profileMappings,profileMappings/${mappingId},pushProviders,pushProviders/${providerId},riskEvents,riskProviders,riskProviders/${providerId},roles,roles/${roleId},roles/${roleId}/permissions,roles/${roleId}/subscriptions,roles/${roleId}/targets,roles/${roleId}/targets/catalog/apps,roles/${roleId}/targets/groups,roles/${roleId}/users,' + 
                'sessions,sessions/me,subscriptions,subscriptions/${subscriptionId},templates/sms,threats/configuration,trustedOrigins,trustedOrigins/${trustedOriginId},userFactors,userTypes,userTypes/${typeId},users,users/me,users/${userId},users/${userId}/appLinks,users/${userId}/clients,users/${userId}/factors,users/${userId}/factors/${factorId},users/${userId}/factors/${factorId}/lifecycle/activate,users/${userId}/factors/${factorId}/lifecycle/deactivate,' + 
                'users/${userId}/factors/${factorId}/verify,users/${userId}/grants,users/${userId}/groups,users/${userId}/idps,users/${userId}/lifecycle/activate,users/${userId}/lifecycle/deactivate,users/${userId}/lifecycle/expire_password,users/${userId}/lifecycle/reactivate,users/${userId}/lifecycle/reset_factors,users/${userId}/lifecycle/reset_password,users/${userId}/lifecycle/suspend,users/${userId}/lifecycle/unlock,users/${userId}/lifecycle/unsuspend,users/${userId}/linkedObjects,users/${userId}/roles,users/${userId}/sessions,zones';
            datalist.innerHTML = paths.split(',').map(path => `<option>/api/v1/${path}`).join("") + 
                "<option>/oauth2/default/v1/authorize<option>/oauth2/default/v1/token<option>/oauth2/default/v1/keys<option>/oauth2/default/v1/userinfo<option>/oauth2/default/v1/introspect<option>/oauth2/default/v1/revoke<option>/oauth2/default/.well-known/openid-configuration<option>/oauth2/default/.well-known/oauth-authorization-server" +
                "<option>/oauth2/v1/clients<option>/oauth2/v1/clients/${clientId}<option>/oauth2/v1/keys<option>/oauth2/v1/keys/${keyId}" +
                "<option>/.well-known/okta-organization<option>/.well-known/openid-configuration<option>/.well-known/oauth-authorization-server";
            ...
            form.onsubmit = function () {
                ...
                if (urlValue.match(/\${.*}/) && location.pathname.match("/admin/(app|group|user)/")) {
                    ...
                    urlValue = urlValue.replace(/\${[^}]+}/g, id);
                }
                requestJSON({url: urlValue, method: method.value, data: data.value}).then((objects, status, jqXHR) => {
                    $(results).html("<br>");
                ...
                }).fail(jqXHR => $(results).html("<br>Status: " + jqXHR.status + " " + jqXHR.statusText + "<br><br>Error:<pre style='font-family: Monaco, Consolas, \"Lucida Console\", monospace; white-space: pre-wrap; word-break: break-all;'>" + e(JSON.stringify(jqXHR.responseJSON, null, 4)) + "</pre>"));
                return false; // Cancel form submit.
            };
        });
    }
        ...
        objects.forEach(row => {
            ...
            for (const p of ths) {
                ...
                if (p == "id") {
                    ...
                    v = "<pre style='font-family: Monaco, Consolas, \"Lucida Console\", monospace; white-space: pre-wrap; word-break: break-all;'>" + e(JSON.stringify(v, null, 4)) + "</pre>";
                } else {
                    v = e(v);
                }
                tds.push("<td>" + v);
    ...
    function formatPre(s, url, addId) {
        return "<pre style='font-family: Monaco, Consolas, \"Lucida Console\", monospace; white-space: pre-wrap; word-break: break-all;'>" + s.replace(/"id": "(.*)"/g, (match, id) => id.startsWith('<') ? match : `"id": "<a href="${url}${addId ? '/' + id: ''}">${id}</a>"`) + "</pre>";
    }
    function linkify(s) {
    ...
    */
    ...
    // Listen to message from background.js (compatible with both Chrome and Firefox).
    const browserAPI = typeof browser !== 'undefined' ? browser : chrome;
    if ((window.chrome || window.browser) && browserAPI.runtime && browserAPI.runtime.onMessage) {
        browserAPI.runtime.onMessage.addListener(function (request, _, sendResponse) {
            getJSON("/api/v1/groups").then(groups => {
                groups = groups
